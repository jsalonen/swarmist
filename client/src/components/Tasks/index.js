...
const Tasks = inject('nodeStore')(observer(
  class Tasks extends Component {
    render() {
      const { nodeStore } = this.props;

      if(!nodeStore.tasks || !nodeStore.tasks || !nodeStore.selectedServices.length) {
        return (<div></div>);
      } else {
        const filteredTasks =
          nodeStore.tasks.filter((task) => 
            nodeStore.selectedServices.indexOf(task.ServiceID) !== -1
          );
